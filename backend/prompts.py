...
ОДНА ПОЗИЦИЯ В ОДНОЙ КОМПАНИИ = ОДИН БЛОК. Если человек работал в одной \
компании на одной должности, но вёл несколько проектов/продуктов — это \
ОДИН блок, а не несколько. Разделяй только если РАЗНЫЕ должности или РАЗНЫЕ компании.
Для каждого блока:
...
ВАЖНО: Перед определением ролей ОБЯЗАТЕЛЬНО поищи в интернете реальные \
вакансии на hh.ru по ключевым навыкам кандидата. Названия ролей должны \
точно соответствовать тому, как их пишут в реальных вакансиях на \
российском рынке (hh.ru, habr career). Не выдумывай англицизмы — \
смотри как реально называются позиции в вакансиях.

Для каждой роли укажи:
- Название роли (как пишут в реальных вакансиях на hh.ru)
...
- Вплетай технологии/инструменты прямо в текст буллетов \
(«Спроектировал на gRPC/REST API», а не отдельным списком) — \
это и ATS-ключевые слова, и контекст что кандидат реально юзал стек
